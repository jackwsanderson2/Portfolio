...
class GetConformedVariables:
    ...
    @staticmethod
    def get_source_table(notebook_path):
        return GetConformedVariables.get_variable(
            notebook_path,
            "SOURCE_TABLE"
        )

    @staticmethod
    def get_target_table(notebook_path):
        return GetConformedVariables.get_variable(
            notebook_path,
            "TARGET_TABLE"
        )
    ...
    @staticmethod
    def get_class_name(notebook_path):
        return GetConformedVariables.get_variable(
            notebook_path,
            "class"
        )

class GetReportingVariables:
    @staticmethod
    def get_variable(notebook_path, variable_name):
        nb = str(nbformat.read(notebook_path, as_version=4))
    ...
    @staticmethod
    def get_view_name(notebook_path):
        return GetReportingVariables.get_variable(
            notebook_path,
            "CREATE OR REPLACE VIEW {catalog}.`{env}-ds-mgmtinfo-neu-dbr-01`"
        )

    @staticmethod
    def get_division_filter(notebook_path):
        nb = str(nbformat.read(notebook_path, as_version=4))
        return 'DIV-16' in nb or 'DIV-15' in nb
            
class GetJobVariables:
    # command: py -m pip install pyyaml
     
    @staticmethod
    def get_yaml(job_path):
        ...
        for job_id, job in jobs.items():
            ...
            for task in job.get("tasks", []):
                print(f"  Task: {task.get('task_key')}")

                notebook = task.get("notebook_task")
                if notebook:
                    print(f"    Notebook: {notebook.get('notebook_path')}")
    ...
    def get_job_data(job_path):
        ...
        return data.get("resources", {}).get("jobs", {})

    @staticmethod
    def get_task_data(job_path, task_key):
        data = GetJobVariables.get_job_data(job_path)

        tasks_lookup = {
            task["task_key"]: task
            for task in data["create_views"]["tasks"]
        }

        return tasks_lookup[task_key]
...
class Checks ():
    ...
    def tbl_conformed_naming(tbl_name):
        """
        Checks variable starts with prefix 'tbl_conformed_'.
        """

        prefix = 'tbl_conformed_'
        if tbl_name.startswith(prefix):
            return True
        else:
            return False

    def vw_reporting_naming(tbl_name):
        """
        Checks variable starts with prefix 'vw_reporting_'.
        """

        prefix = 'vw_reporting_'
        if tbl_name.startswith(prefix):
            return True
        else:
            return False
    ...
    def view_extract(input):
            """
            Extracts name of data.
            """
            search_term = 'vw_reporting_'
            position = input.find(search_term)
            name = input[position+11:]
    ...
    @staticmethod
    def get_country(variable_name):
        return variable_name.rsplit("_", 1)[1]